          ...
          <div className="flex items-center gap-1 sm:gap-2 shrink-0">
            ...
            {isAuthenticated ? (
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-56 mt-2">
                  <div className="flex items-center gap-3 p-3 bg-gray-50/50">
                    ...
                    <div className="flex flex-col overflow-hidden">
                      <p className="text-sm font-bold text-[#4a2e2b] truncate">{user?.ad || user?.name}</p>
                  ...
                  {(user?.rol === 'admin' || user?.role === 'admin') && (
                    <>
                      <DropdownMenuSeparator />
                      <DropdownMenuItem asChild className="cursor-pointer">
                        <Link href="/admin" className="font-bold text-[#4a2e2b]">Admin Paneli</Link>
                      </DropdownMenuItem>
                    </>
                  )}
                  <DropdownMenuSeparator />
              ...
              <div className="hidden sm:flex items-center gap-2">
                ...
                <Link href="/kayit">
                  <Button size="sm" className="bg-[#4a2e2b] hover:bg-[#3a2422] text-white">
                    Üye Ol
                  </Button>
                </Link>
              </div>
        ...
        <div className="container mx-auto px-4">
          <nav className="flex items-center gap-8 py-3">
            <DropdownMenu>
              ...
              <DropdownMenuContent align="start" className="w-48 mt-2 p-2 shadow-xl border-gray-100 rounded-xl">
      ...
      {mobileMenuOpen && (
        <div className="md:hidden border-t border-gray-100 bg-white">
          ...
          <nav className="p-4 space-y-1">
            {[...DROPDOWN_CATEGORIES, ...NAV_CATEGORIES.filter(
              (n) => !DROPDOWN_CATEGORIES.find((d) => d.slug === n.slug)
            )].map((cat) => (
              <Link
                key={cat.slug}
                href={`/kategori/${cat.slug}`}
          ...
          {!isAuthenticated && (
            <div className="p-4 border-t border-gray-100 flex gap-3">
              ...
              <Link href="/kayit" className="flex-1">
                <Button className="w-full bg-[#4a2e2b] hover:bg-[#3a2422] text-white">Üye Ol</Button>
              </Link>
            </div>
          )}
        </div>
